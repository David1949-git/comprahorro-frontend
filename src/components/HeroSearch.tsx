import { Search, X, ArrowLeft, Radar } from "lucide-react";
...
const HeroSearch = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const params = new URLSearchParams({ q: searchQuery.trim(), lat, lon });
            ...
            <div className="text-center space-y-12" style={{ animation: "fade-in-up 0.6s ease-out" }}>
              <div className="mx-auto max-w-4xl">
                <div className="bg-white/70 backdrop-blur-md shadow-lg rounded-[2rem] p-4 inline-block mb-6">
                  <img src={piggyLogo} alt="Logo ComprAhorro" className="h-32 md:h-48 w-auto object-contain mx-auto rounded-2xl" />
                </div>
                <h2 className="text-4xl md:text-6xl font-black tracking-tight text-foreground mb-2">ComprAhorro</h2>
              ...
              <div className="space-y-10">
                <h1 className="text-3xl md:text-5xl font-black tracking-tight text-slate-800">Nosotros buscamos, ¡tú ahorras!</h1>
                <div className="mx-auto flex max-w-5xl flex-col md:flex-row items-stretch justify-center gap-6">
                  {[["1. COMPARA","Precios reales"],["2. ELIGE","Sin presiones"],["3. COMPRA","Directo sin intermediarios"]].map(([t,s])=>(
                    <div key={t} className="flex-1 rounded-2xl border border-primary/10 bg-white/60 backdrop-blur-sm px-6 py-8 shadow-soft hover:shadow-md transition-shadow">
                      <p className="text-lg font-black tracking-wider text-primary">{t}</p>
                      <p className="mt-2 text-sm font-semibold text-slate-500 uppercase tracking-tight">{s}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <form onSubmit={handleSearch} className="mx-auto mt-8 max-w-3xl">
                <div className={`relative transition-all duration-300 ${isFocused ? 'scale-[1.02]' : ''}`}>
                  <div className={`relative flex items-center bg-background border-2 rounded-2xl transition-all duration-300 ${isFocused ? 'border-primary shadow-xl' : 'border-input hover:border-primary/50 shadow-md'}`}>
                    ...
                    <button type="submit" disabled={!searchQuery.trim()} className="bg-primary text-primary-foreground px-10 py-6 rounded-xl font-bold text-lg hover:opacity-90 transition-all mr-2 disabled:opacity-50">
                      BUSCAR
                    </button>
                  </div>
                </div>
            ...
            <div className="flex flex-col items-center justify-center py-24 space-y-6">
              <div className="relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <Radar size={28} className="text-primary animate-pulse" />
                </div>
              </div>
              <div className="text-center space-y-2">
                <p className="text-xl font-bold text-foreground">El radar está buscando...</p>
                <p className="text-sm text-muted-foreground">💡 Si no ves resultados, intenta con un término más corto.</p>
              </div>
            </div>
          )}
          ...
          {hasSearched && !isSearching && (
            <div className="space-y-6">
              <div className="flex items-center gap-4">
                <button onClick={() => { setHasSearched(false); setSearchResults([]); setSearchQuery(""); setVeredicto(""); }}
                ...
                <form onSubmit={handleSearch} className="flex-1 max-w-2xl">
                  <div className="relative flex items-center bg-background border-2 rounded-2xl border-input">
                    ...
                    <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                      placeholder="Buscar productos..." className="flex-1 px-4 py-4 bg-transparent focus:outline-none" />
                    <button type="submit" className="bg-primary text-primary-foreground px-6 py-4 rounded-xl font-semibold mr-2">Buscar</button>
                  </div>
                </form>
              </div>
              ...
              <div className="text-center">
                <h2 className="text-2xl font-bold text-foreground">
                  {searchResults.length > 0 ? `${searchResults.length} resultados encontrados` : 'No se encontraron resultados'}
                </h2>
              </div>
